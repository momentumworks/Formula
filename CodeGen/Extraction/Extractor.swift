...
public struct Extractor {
  ...
  public static func parseDirectory(path: String) -> [TypeName : Object] {
    let files = NSFileManager.defaultManager().enumeratorAtPath(directory)?.allObjects as! [NSString]

    return files.filter{ $0.pathExtension == "swift" }.reduce([TypeName : Object]()) { objs, filePath in
      let file = File(path: "\(directory)/\(filePath)")!
